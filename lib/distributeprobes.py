    ...
    for i, sites in enumerate(siteCandidates):
        ...
        try:
            ...
            while current >= siteCandidates[i][0][0]:   # possible to squeeze more in
                ...
                idx_max_start = np.amax(np.append(np.nonzero(siteCandidates[i][0] <= current), -1))
                idx_min_end = np.amin(np.append(np.nonzero(siteCandidates[i][1] >= current), len(seq)))
                while idx_max_start != idx_min_end and current >= siteCandidates[i][0][0]:
                    current -= 1
